        ...
        for update in range(self.resume_at, self.updates):
            ...
            if episode_result:
                self.monitor.log((("{:4} sec={:2} full reward={:.2f} reward={:.2f} std={:.2f} length={:.1f} std={:.2f} ") +
                    (" value={:3f} std={:.3f} advantage={:.3f} std={:.3f} sequence length={:3} learned_std={:3f} std={:3f}")).format(
                    ...
                    torch.mean(self.sampler.buffer.advantages), torch.std(self.sampler.buffer.advantages), self.sampler.buffer.actual_sequence_length, torch.mean(self.sampler.buffer.std), torch.std(self.sampler.buffer.std)) +
                    " " + formatted_string)
            else:
